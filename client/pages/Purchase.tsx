...
import { purchaseAPI, partyAPI, productAPI } from "@/lib/api";
import { Purchase } from "@shared/api";
...
const paymentMethods = ["Cash", "Bank Transfer", "Cheque", "Card", "Credit"];
...
export default function Purchase() {
  ...
  const createMutation = useMutation({
    mutationFn: (d: any) => purchaseAPI.create(d),
    onSuccess: () => {
  ...
  const handleSavePurchase = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.purchaseNumber || !formData.supplierId || formData.items.length === 0) {
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Record and manage supplier purchases</p>
          </div>
        ...
        {showForm && (
          <div className="bg-white rounded-lg border border-border p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-foreground">Create New Purchase Order</h2>
            ...
            <form onSubmit={handleSavePurchase} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="text" name="purchaseNumber" value={formData.purchaseNumber}
                    onChange={handleInputChange} placeholder="PUR-001"
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground">PO #</th>
                  ...
                  {filteredPurchases.length > 0 ? (
                    filteredPurchases.map(purchase => (
                      <tr key={purchase.id}
                        ...
                        <td className="px-6 py-4 text-sm text-foreground">{purchase.supplierName}</td>
                        ...
                        <td className="px-6 py-4 text-sm">
                          <Select value={purchase.status}
                            onValueChange={v => handleStatusChange(purchase.id, v)}>
                      ...
                      <td colSpan={7} className="px-6 py-8 text-center">
                        <p className="text-muted-foreground">
                          {searchTerm || filterStatus !== "all"
                            ...
                            : "No purchases yet. Click \"Create Purchase\" to get started."}
                        </p>
                      </td>
                    </tr>
                  )}
